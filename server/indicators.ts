...
/** Bollinger Bands */
export function bollingerBands(prices: number[], period = 20, stdDev = 2): { upper: number; middle: number; lower: number } {
...
export function macd(prices: number[]): { macdLine: number; signal: number; histogram: number } {
  ...
  // For simplicity, use the MACD line directly
  const signal = ema(prices.slice(-9), 9) > ema(prices.slice(-26), 26) ? macdLine * 0.8 : macdLine * 1.2;
